(function (exports, undefined) {
	...
	ModuleLoader.prototype._exec = function () {
		...
		for (var modulePath in this._depTable) {
			...
			vars += ['var ', varName, ' = JSON.parse(\'', JSON.stringify(loader._instance, iterate), '\', function (key, value) { if (value  && typeof value === "string"  && value.substr(0,8) == "function") { var startBody = value.indexOf("{") + 1; var endBody = value.lastIndexOf("}"); var startArgs = value.indexOf("(") + 1; var endArgs = value.indexOf(")"); return new Function(value.substring(startArgs, endArgs), value.substring(startBody, endBody)); } return value; });'].join('');
		}
		console.log(vars);
